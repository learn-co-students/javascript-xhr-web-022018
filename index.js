function getRepositories(){
  const req = new XMLHttpRequest()  //creating a new instance of an XMLHttpRequest
  let address = 'https://api.github.com/users/octocat/repos'
  req.addEventListener("load",showRepositories)
  req.open("GET",address)
  req.send()
}

function showRepositories(event,data){
  //this is set to the XMLHttpRequest object that fired the event
  // console.log(this.responseText); //what does 'this' refer to?????????????
    // 'this' will be our req object inside our callback function.
    //  so this is req.responseText, see how this func is a callback for above

  //The way we tell the interpreter that we're working with JSON is to parse it with JSON.parse.
  var repos = JSON.parse(this.responseText)
  console.log(repos);
  const repoList = `<ul>${repos.map(r=>'<li>'+r.name+
    ' - <a href="#" data-repo="' + r.name+
    '" onclick="getCommits(this)">Get Commits</a>' +
    '<li>').join('')}</ul>`

  document.querySelector("#repositories").innerHTML = repoList
  // console.log(repoList); // remove not needed
}


function getCommits(el) { //element
  const name = el.dataset.repo;
  const req =  new XMLHttpRequest()
  req.addEventListener("load",showCommits)
  req.open("GET", 'https://api.github.com/repos/octocat/' + name + '/commits')
    req.send()
}
function showCommits() {
  const commits = JSON.parse(this.responseText)
  const commitsList = `<ul>${commits.map(commit => '<li><strong>' + commit.author.login + '</strong> - ' + commit.commit.message + '</li>').join('')}</ul>`
  document.getElementById("commits").innerHTML = commitsList
}
